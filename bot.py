...
OPERATION, INPUT, SUM, SUB, MULTH, DIV = range(6)
...
def op(update, _):
    ...
    update.message.reply_text(
        "Выберите операцию, которую ты хочешь выполнить",
    reply_markup=markup_key,)
    return INPUT
...
com_handler = ConversationHandler(entry_points=[CommandHandler('start', start)],states=
    {OPERATION: [messagehandler(filters.regex('^(Сумма|Разность|Умножение|Деление)$'), op)],  # этапы разговора, каждый со своим списком обработчиков сообщений
